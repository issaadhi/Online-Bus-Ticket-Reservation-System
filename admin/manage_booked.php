...
$qry = $conn->query("SELECT * FROM booked_ticket where id = ".$_GET['id']);
	...
	$('#book-ticket').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp ==1 ){
					$('.modal').modal('hide')
					alert_toast("Booked Ticket successfully updated.","success")
					setTimeout(function(){
						location.reload();
					},1500)
				}
