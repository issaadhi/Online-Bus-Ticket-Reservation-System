<div class="container-fluid">
	...
	<form action="" id="book-bus">
		...
		<div class="form-group row" id="qty">
			...
			<button class="btn btn-secondary btn-block" type="button" id="go">Go</button>
			</div>
			<div class="col-md-2">
			<label for="" class="control-label">&nbsp;</label>
...
<script>
	$('#go').click(function(){
		...
		if('<?php echo $_GET['max'] ?>' < $('#count').val()){
			alert("The number of person can't be greater than the available seats.")
					end_load()
			return false;
		}
		$.ajax({
	...
	$('#book-bus').submit(function(e){
		e.preventDefault()
		start_load()
		$.ajax({
			url:'admin/ajax.php?action=book_ticket',
			method:"POST",
			data:$(this).serialize(),
			success:function(resp){
				if(resp ==1 ){
					$('.modal').modal('hide')
					end_load()
					alert_toast("Bus successfully booked.","success")
				}
			}
		})
	})
